...
def train_deep_monte_carlo_parallel_with_logging(
    number_of_episodes,
    learning_rate=1e-4,
    epsilon_start=0.9,
    epsilon_end=0.05,
    ...
    try:
        while episode_index < number_of_episodes:
            ...
            tasks_to_send = []
            local_actor_index = 0
            while local_actor_index < number_of_actor_processes and episode_index + local_actor_index < number_of_episodes:
                current_episode_index = episode_index + local_actor_index

                if current_episode_index < decay_episodes:
                    epsilon = epsilon_end + (epsilon_start - epsilon_end) * (
                        1.0 - float(current_episode_index) / float(decay_episodes - 1)
                    )
                else:
                    epsilon = epsilon_end
                ...
                tasks_to_send.append(
                    {
                        "q_network_state_dictionary_bytes": q_network_state_dictionary_bytes,
                        "epsilon": epsilon,
                        "episode_index": current_episode_index,
                        "decay_episodes_enemy": decay_episodes_enemy,
                    }
                )
                local_actor_index += 1

            for task_dictionary in tasks_to_send:
                actor_task_queue.put(task_dictionary)

            received_result_list = []
            received_results_count = 0
            while received_results_count < len(tasks_to_send):
                received_result_list.append(learner_result_queue.get())
                received_results_count += 1

            received_result_list.sort(key=lambda result_dictionary: result_dictionary["episode_index"])

            for result_dictionary in received_result_list:
                current_episode_index = int(result_dictionary["episode_index"])

                episode_ally_state_actions = result_dictionary["episode_ally_state_actions"]
                episode_enemy_state_actions = result_dictionary["episode_enemy_state_actions"]
                ally_return_list = result_dictionary["ally_return_list"]
                enemy_return_list = result_dictionary["enemy_return_list"]
                final_reward_ally = result_dictionary["final_reward_ally"]
                final_reward_enemy = result_dictionary["final_reward_enemy"]
                ...
                if evaluation_interval is not None and evaluation_interval > 0:
                    if (current_episode_index + 1) % evaluation_interval == 0:
                        q_network.eval()
                ...
                if csv_writer is not None:
                    if current_episode_index < decay_episodes:
                        epsilon_for_csv = epsilon_end + (epsilon_start - epsilon_end) * (
                            1.0 - float(current_episode_index) / float(decay_episodes - 1)
                        )
                    else:
